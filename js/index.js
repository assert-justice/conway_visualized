"use strict";
function drawLineHoriz(sim, x, y, length) {
...
function drawBox(sim, x, y, width, height) {
...
function drawNegBox(sim, x, y, width, height) {
    for (let i = 0; i < width; i++) {
        for (let j = 0; j < height; j++) {
            if (onGrid(sim, x + i, y + j)) {
...
function safeSet(sim, x, y, value) {
    if (onGrid(sim, x, y)) {
        sim.setCell(x, y, value);
    }
}
function drawBoxEmpty(sim, x, y, width, height) {
    for (let i = 0; i < width; i++) {
...
function countNeighbors(sim, x, y) {
    //
    // for(let i = 0; i < )
    let adj = 0;
    ...
    for (let i = -1; i < 2; i++) {
        for (let j = -1; j < 2; j++) {
            ...
            const cx = x + i;
            const cy = y + j;
            if (onGrid(sim, cx, cy) && sim.getCell(cx, cy))
                adj++;
        }
    }
    return adj;
